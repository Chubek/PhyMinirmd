...
void calcMinimizersFun()
{ // before calling this method, MUST set rid_pthread = 0
...
inline void processCluster()
{
	...
	{
		if (isPE)
			threadVec.push_back(std::thread(processClusterPEFun));
		else
			threadVec.push_back(std::thread(processClusterSEFun));
	}
	std::for_each(threadVec.begin(), threadVec.end(), [](std::thread &thr)
				  { thr.join(); });
	threadVec.clear();
...
inline void processLargeCluster()
{
	...
	for (size_t i = 0; i < sgvnl; ++i)
	{
		...
		{
			if (isPE)
				threadVec.push_back(std::thread(processLargeClusterPEFun));
			else
				threadVec.push_back(std::thread(processLargeClusterSEFun));
		}
		std::for_each(threadVec.begin(), threadVec.end(), [](std::thread &thr)
					  { thr.join(); });
		threadVec.clear();
